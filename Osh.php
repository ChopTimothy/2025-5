...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if (!empty($_SESSION['login'])) {
            ...
            $sql_2 = "DELETE FROM application_languages WHERE application_id = ?";
            $pdo->prepare($sql_2)->execute([$_SESSION['uid']]);
        } else {
            $login = uniqid('user_');
            ...
            $sql_3 = "INSERT INTO applications
        ...
        $sql_4="INSERT INTO application_languages (application_id, language_id)
            SELECT ?, id FROM languages WHERE name = ?";
        $lang_stmt = $pdo->prepare($sql_4);
